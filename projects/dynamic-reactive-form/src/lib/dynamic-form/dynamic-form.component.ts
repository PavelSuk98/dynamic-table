...
export class DynamicFormComponent implements OnInit {
   ...
   */
  @Input({ required: true}) fieldset!: Field[]; // Required
  ...
  initializeFormControl(field: Field): UntypedFormControl {
    let value;
    ...
    if (field.type === 5) {
      ...
      if (field.defaultValue === false) {
        const fieldIndex = this.fieldset.findIndex(x => x.name === field.name);
        this.hideChildren(fieldIndex);
      }
    }
     ...
     */
    const validation = field.validation ? field.validation : new Array();
